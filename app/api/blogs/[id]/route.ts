...
export async function PUT(
  ...
  try {
    ...
    const existing: Blog | null = (await dbGet(
    ...
    // Get updated blog
    const updated: Blog = (await dbGet("SELECT * FROM blogs WHERE id = ?", [
      id,
    ])) as Blog;
...
export async function GET(
  ...
  try {
    const { id } = await params;
    const blog: Blog | null = (await dbGet("SELECT * FROM blogs WHERE id = ?", [
      id,
    ])) as Blog | null;
